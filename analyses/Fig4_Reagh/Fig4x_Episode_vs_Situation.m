...
%% from here:
    ...
    for ll = 1: size(allVectors ,2)
        ...
        for pl = 1:length(places)
            ...
            tmp = dataStruct.(keyTommy).video;
            if strcmp(loo_label, key)
                tmp(:,loo_nr) = [];
            end
            dataTommy = tmp;  % features x 3

            keyLisa = sprintf('lisa_%s', places{pl});
            tmp = dataStruct.(keyLisa).video;
            if strcmp(loo_label, keyLisa)
                tmp(:,loo_nr) = [];
            end
            dataLisa = tmp;    % features x 3
    
            % All pairwise comparisons between Tommy and Lisa videos
            for i = 1:size(dataTommy,2)
                for j = 1:size(dataLisa,2)
                    Vec1 = dataTommy(:, i);
                    Vec2 = dataLisa(:, j);
        ...
        for ff = 1 : num_components
            % ff
            [w_x, w_y, lbd3i] = crm(Cov_xx_sameEpisode, ...
                Cov_yy_sameEpisode, Cov_sameEpisode, Cov_samePlaceDifferenPerson, f=ff, gamma=0.001);
    ...
    filename = fullfile(sj_path, 'correlationMatrix_episode_cv');
